const HistoryView = {
  render(container, folderId, noteName) {
    ...
    let html = `<h2 class="page-title">📊 Score History</h2>`;
    ...
    if (attempts.length === 0) {
      ...
      for (const a of attempts.slice().reverse()) {
        ...
        const color = a.percentage >= 80 ? 'var(--success)' : a.percentage >= 50 ? 'var(--warning)' : 'var(--danger)';
        html += `
    ...
    html += `
      <div style="margin-top:1.5rem;">
        ...
        <button class="btn btn-outline btn-sm" style="margin-left:0.5rem;" onclick="router.navigate('#/folder/${encodeURIComponent(folderId)}')">← Back to ${folder?.name||'Folder'}</button>
      </div>`;
