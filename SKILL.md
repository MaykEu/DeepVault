...
- **5-15 questions per note MANDATORY.** 1-2 question quizzes are UNACCEPTABLE — the user explicitly called this out. Mix MC (2-4 options) and text input.
...
- **FIELD NAMES MUST BE FULL:** `question` NOT `q`, `options` NOT `o`, `correctAnswer` NOT `a`, `explanation` NOT `e`. Abbreviated fields cause every quiz to show "undefined" — the quiz engine reads `q.question` which is undefined when the field is `q.q`.
- **JSON ESCAPING:** Do NOT backslash-escape single quotes (`\'`). `\'` is INVALID JSON — single quotes need no escaping. Only `\"` and `\\` need backslashes. Invalid escaping causes `SyntaxError: Unterminated string`. Use `json.dumps()` which handles this correctly — never build JSON strings manually with string concatenation.
- **BATCHES MUST BE INSERTED AFTER WRITING:** Writing quiz JSON to batch files is NOT complete. The Quiz creator wrote 651 questions across 17 files that sat dead on disk while the live website still had 1Q quizzes. After writing batch files, you MUST insert them into `data.js` BEFORE the `const REFERENCE` block. Run `node --check js/data.js` after every insertion. Run `python quiz-cli.py verify` to confirm 0 errors.
- **AFTER ALL QUIZ WORK:** Run `python quiz-cli.py verify`. Must return 0 errors AND 0 warnings before work is complete. Run `node test.js` — must pass 20/20.
...
- `data/quizzes.json` — all QUIZ_DATA + QUIZ_NOTES. **Quiz creators write here safely — can't corrupt notes.**
- `data/folders.json` — folder structure per domain.
- `data/reference.json` — glossary, guide, learning paths.

Build process: `python build.py` validates all quizzes (explanations present, MC answers in options) → generates `js/data.js`. **If validation fails, build ABORTS — never ships broken data.**

Quiz creator workflow: write ONLY to `data/quizzes.json`. Run `python build.py` to test. Never touch `notes.json`, `folders.json`, or `reference.json`.

```
data/
├── notes.json      ← Note content (vault → build.py)
├── quizzes.json    ← Quiz questions (Quiz creator writes here — safe)
...
convert.py          ← Legacy — replaced by build.py
js/data.js          ← GENERATED — never edit manually
```

## Architecture

**VAULT FIRST — ALWAYS.** The Obsidian vault at `C:\Users\sofia\Documents\Obsidian\Game Development\` is the canonical source of all content. Every change to a note, table, link, or section MUST happen in the vault `.md` file FIRST, then sync to `data.js`. Never edit content in `data.js` alone — it will be overwritten by the next `convert.py` rebuild and your changes will be lost forever.

## Scalable Architecture (2026-07-04)

data.js is built from 4 independent JSON files in `data/` — never edited directly:

```
data/
├── quizzes.json      ← Quiz creator writes here via quiz-cli.py. Can't corrupt notes.
├── notes.json        ← build.py populates from vault (.md files)
├── folders.json      ← build.py populates from vault folder structure
├── reference.json    ← build.py populates from vault (Glossary, Guide, LPs)
└── build.py          ← Validates all 4 JSON → generates js/data.js
```

**build.py** validates each JSON file before combining. Schema rules enforced:
- quizzes.json: every question has `explanation`, MC `correctAnswer` in `options`, min 5Q per note, no abbreviated field names
- notes.json: every entry has `folder` field
- folders.json: root `_root` key + subfolder keys
- reference.json: 9 required entries (Guide, Glossary, 6 LPs, LP index)

If any file fails validation, build FAILS — never ships broken data.

**Quiz creator writes to `data/quizzes.json` via `quiz-cli.py add`** — after every batch, run `build.py` to regenerate `js/data.js`, then `node --check js/data.js`.

**Extraction from existing data.js:** Use `data/extract.js` — executes data.js in a Function scope (replacing `const` with `var`) to capture all globals, then writes the 4 JSON files.
...
9. **Empty header cells in tables are DROPPED by the renderer**, misaligning columns. A `| | **FName** | ...` table loses the empty header, shifting all headers left by one. Fix: use a real label like `| **Property** |`. Never use `| &nbsp; |` — the entity renders as literal text.

10. **Lists**: Detect `^\\d+\\.\\s+` for `<ol>`, `^[-*]\\s+` for `<ul>`. Wrap items in `<li>`.
...
## Quiz Button on Learn Page

Each Learn page shows a 🏆 Quiz button (if the note has quiz data) and a Bookmark toggle. **Layout: Back link on LEFT, Bookmark + Quiz on RIGHT** (`margin-left:auto` in a flex container). User corrected the previous clustered-left layout as bad UX.

```js
'<div style="display:flex;align-items:center;margin-bottom:0.5rem;">' +
  '<a href="#" class="back-link" style="margin-bottom:0;" onclick="...">← Back</a>' +
  '<span style="margin-left:auto;display:inline-flex;align-items:center;gap:4px;">' +
    '<span class="bookmark-star" onclick="...">☆</span>' +
    '<a class="btn btn-quiz btn-sm" onclick="...">🏆 Quiz</a>' +
  '</span>' +
'</div>'
...
## verification step — Mandatory After Every Change

**Every functional change MUST be verified by the verification step (session `20260703_185004_3c4131`).** User explicitly called this out: "you never check with verifier." Pattern: make changes → commit → dispatch verification step via `hermes chat --resume 20260703_185004_3c4131 -q "VERIFY: ..." --yolo` → fix issues → re-verify. Never declare "done" without verification step confirmation.

## Sub-Agent Non-Stop Pattern

**When dispatching a Quiz creator for multi-batch tasks, always include explicit non-stop instructions.** Quiz creators naturally break work into batches and ask "Continue?" — this wastes sessions. Use: "DO NOT PAUSE or ask to continue. ONLY stop when ALL notes are done."

## Table Rendering — Empty First Column Bug

**Tables with an empty first header cell (`| | **Col1** |`) render with headers shifted left.** The renderer drops the empty header. **Fix: give the first column a label** (`| **Property** | **FName** |`). Do NOT use `&nbsp;` — renders as literal text.
...
**One agent per task.** If using multiple AI agents, only one should write to data/quizzes.json at a time. Parallel writes cause conflicts and lost work.

**Write to quizzes.json, then build.** Write quizzes to `data/quizzes.json`, run `python build.py` to rebuild data.js. The build validates all quizzes before generating output — errors fail the build. Never write data.js directly.
...
**The most common data.js corruption:** Appending QUIZ_DATA entries after the REFERENCE block truncates it. Symptom: Glossary, Guide, and all 6 Learning Paths vanish. Fix: write to `data/quizzes.json`, run `python build.py` to rebuild cleanly.
...
## Quiz Creation — Use quiz-cli.py

**PREFERRED METHOD: `python quiz-cli.py add quizzes.json`** — handles insertion, QUIZ_NOTES rebuild, and validation automatically. Write quizzes to `data/quizzes.json`, run `python build.py` to validate and rebuild. See `references/quiz-creation-workflow.md`.
...
## Quiz creator Instructions — MUST Include These Verbally

When dispatching the quiz creation step (session `20260703_203312_ac4699`) for quiz work, these exact instructions MUST be in the prompt or the quiz creation step WILL make these mistakes. User blames YOUR instructions, not the quiz creation step:

```
1. FIELD NAMES: Use FULL names — question, options, correctAnswer, explanation, acceptableAnswers. NEVER abbreviate to q, o, a, e. Abbreviated fields cause every quiz to show "undefined".

2. JSON ESCAPING: Use json.dumps() for ALL JSON output. NEVER manually build JSON strings. Do NOT backslash-escape single quotes (\\' is invalid JSON).

3. BATCH-UNTIL-INSERT: Write batches to JSON files, THEN use quiz-cli.py add to insert into data/quizzes.json, THEN run build.py to regenerate js/data.js. Keyword: "INSERT ALL BATCHES IMMEDIATELY after writing — never leave them on disk."

4. NON-STOP: "DO NOT PAUSE or ask to continue. Process ALL notes until complete."

5. VERIFY: After every batch, dispatch verification step. "Fix errors before next batch. verification step must confirm CLEAN."
## Sub-Agent Pitfalls

When using AI coding agents:

**Agents do not check file paths.** They work in isolated environments with no access to your working directory. Always provide absolute paths in the `context` field.
...
**Six common link-fix patterns are in `references/link-fix-checklist.md`.** Follow the checklist in order — it covers table parser bugs, greedy regexes, pipeline overview insertions, browser caching, and JSON escape gotchas.
...
- `storage.js` — `toggleBookmark(fid, nn)`, `isBookmarked(fid, nn)`, `getBookmarks()`. **Auto-syncs to Gist when connected.**
...
- `learn.js` — bookmark toggle on individual Learn page header (next to Back + Quiz buttons)

### GitHub Gist Cloud Sync

Cross-device progress sync via private GitHub Gist. See `references/gist-sync.md` for full architecture. Summary:

- **Token:** Classic GitHub personal access token with `gist` scope only. Stored in `localStorage` (`dv_gist_token`). Private, encrypted in transit, invisible to anyone without the token.
- **Storage (`storage.js`):** `syncToGist()` — POST/PATCH to `api.github.com/gists`, creates private gist on first use. `loadFromGist()` — GET all gists, find by filename, merge with localStorage. Merge logic: union attempts, max score per attempt ID, union bookmarks, latest recents.
- **Dashboard (`dashboard.js`):** ☁️ Sync button in toolbar (replaced Export/Import). `showSync()` — connected state shows 🟢 status dot + Disconnect button; disconnected state shows token input + classic token instructions. `connectGist()` / `doSync()` handle the flow.
- **Auto-sync:** After every quiz completion (`quiz-engine.js`) and bookmark toggle (`storage.js` `toggleBookmark`). Silent — no UI feedback needed.
- **`.nojekyll`:** Required at repo root for GitHub Pages to skip Jekyll processing (2MB data.js fails Jekyll build).
